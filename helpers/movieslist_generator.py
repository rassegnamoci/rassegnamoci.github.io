...
df = df[['Data proiezione', 'Film',	'Selezionatore', 'Tema', 'Anno', \
         'Regista/i', 'Durata', 'Paese/i di produzione', 'Link IMDb']]
...
data = {'table': df.to_html(index = False, formatters={'Link IMDb': lambda x: \
        '<a href="'+x+'" target="_blank">'+x+'</a>'}).replace('&lt;', \
        '<').replace('&gt;','>')}
